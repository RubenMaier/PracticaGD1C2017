...
alter function ejer1 (@articulo varchar(8), @deposito varchar(2))
returns varchar(50)
as
begin 
declare @actual int;
...
EXEC @ret = dbo.ejer1 @articulo='00000030', @deposito='00';
select @ret;
...
from GD2015C1.dbo.Item_Factura
	join  GD2015C1.dbo.Factura on item_tipo=fact_tipo and item_numero=fact_numero and item_sucursal=fact_sucursal
...
DECLARE @ret int;
EXEC @ret = dbo.ejer2 @articulo=00001415, @fecha='2012-01-16 00:00:00';
select @ret;
...
begin
	...
	select @gerente= (select top 1 empl_codigo from GD2015C1.dbo.Empleado where empl_jefe is null order by empl_salario desc, datediff(day, getdate(),empl_ingreso) desc);
	select @cant_null =(select count(empl_codigo) from GD2015C1.dbo.Empleado where empl_jefe is null) - 1;
	-- START CURSOR
	...
	begin
	 -------------------------------------------------------------------------
		update GD2015C1.dbo.Empleado set empl_jefe=@gerente where current of empleado_cursor;
...
begin
	...
	begin
	 -------------------------------------------------------------------------
		 ...
		 update GD2015C1.dbo.Empleado set empl_comision=@monto_emp where current of empleado_cursor;
...
--Create table GD2015C1.dbo.Fact_table
--(
...
--monto decimal(12,2)
--)
--Alter table GD2015C1.dbo.Fact_table Add primary key(anio,mes,familia,rubro,zona,cliente,producto)
...
begin transaction
insert into GD2015C1.dbo.Fact_table
select year(fact_fecha), month(fact_fecha), prod_familia, prod_rubro, isnull(depo_zona, 'nul'), fact_cliente, item_producto, sum(item_cantidad), sum(item_precio)
	...
	join GD2015C1.dbo.Stock on prod_codigo=stoc_producto
	join GD2015C1.dbo.Deposito on stoc_deposito=depo_codigo
...
select @index=count(*)
	from GD2015C1.dbo.Item_Factura
	join GD2015C1.dbo.Composicion C1 on item_producto=C1.comp_componente
	and item_cantidad >= C1.comp_cantidad and item_numero=99999999
	group by item_tipo, item_sucursal, item_numero, C1.comp_producto
	having count(*) = (select count(*) from GD2015C1.dbo.Composicion C2
...
BEGIN  
	...
	declare comp_cursor cursor
		for select comp_componente from GD2015C1.dbo.Composicion
	...
	begin
	 ---------------------------------------
		delete GD2015C1.dbo.Item_Factura where item_tipo=@tipo and item_sucursal=@sucursal and item_numero=@numero and item_producto=@comp_producto;		
	 ---------------------------------------
	 fetch next from comp_cursor
		into @comp_producto;
	end
	...
	insert into GD2015C1.dbo.Item_Factura values (@tipo, @sucursal, @numero, @producto, 1, (select prod_precio from GD2015C1.dbo.Producto where prod_codigo=@producto));
	set @index	= @index -1;
END  
...
use GD2015C1;
go
alter trigger tr_ejer9 on GD2015C1.dbo.Item_factura
For update
as
Begin tran
-----------------------------------------------------
...
begin
	...
	declare @deposito as char(2);
		...
		select top 1 @deposito=S2.stoc_deposito from GD2015C1.dbo.Stock S2 where S2.stoc_producto = @item_codigo order by S2.stoc_cantidad desc;				
		-- actualizo el producto padre sea o no compuesto
		update GD2015C1.dbo.Stock set stoc_cantidad= (stoc_cantidad + @cant_deleted - @item_cantidad)  
		...
		declare comp_cursor cursor for
			select comp_producto, comp_componente, comp_cantidad from GD2015C1.dbo.Composicion where comp_producto=@item_codigo;
		...
		begin
			-----------------------------				
			-- actualizo
			update GD2015C1.dbo.Stock set stoc_cantidad= (stoc_cantidad + @cant_deleted*@comp_cantidad - @item_cantidad*@comp_cantidad)  
...
use GD2015C1;
go
create trigger tr_ejer10 on GD2015C1.dbo.Producto
instead of delete
as
Begin tran
		...
		begin
			-----------------------------				
			if exists (select * from GD2015C1.dbo.stock where stoc_producto=@prod_codigo)
			begin
				select 'El producto no se puede borrar ya que posee stock';
			end
			else
			begin
				delete GD2015C1.dbo.Producto where prod_codigo=@prod_codigo;
			end
			-----------------------------
			fetch next from prod_cursor
			into @prod_codigo;
...
begin
	...
	select @cantEmpl=isnull(count(*),0) from GD2015C1.dbo.Empleado where empl_jefe=@jefe;
	if @cantEmpl = 0
	begin
		return 0;
	end
	...
	declare empleado_cursor cursor
		for select empl_codigo from GD2015C1.dbo.Empleado where empl_jefe=@jefe;
	...
	begin
	 ---------------------------------------
		set @cantEmpl = @cantEmpl + GD2015C1.dbo.ObtenerCantEmpleadosDe(@emp_codigo);
	 ---------------------------------------
	 fetch next from empleado_cursor
		into @emp_codigo;
	end
...
--select GD2015C1.dbo.ObtenerCantEmpleadosDe(1)
...
use GD2015C1;
go
alter trigger tr_ejer12 on GD2015C1.dbo.Empleado
...
update GD2015C1.dbo.Empleado set empl_nombre=(empl_nombre + '1')
...
use GD2015C1;
go
create trigger tr_ejer14 on GD2015C1.dbo.Empleado
...
Begin tran
		...
		begin
			...
			select @salario_max=(sum(E2.empl_salario)*20/100) from GD2015C1.dbo.Empleado E2 where E2.empl_jefe=@empl_codigo;
			...
			begin
				update GD2015C1.dbo.Empleado set empl_salario=@empl_salario where empl_codigo=@empl_codigo;
			end			
			-----------------------------
			fetch next from empl_cursor
			into @empl_codigo, @empl_jefe, @empl_salario;			
