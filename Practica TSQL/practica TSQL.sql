...
CREATE FUNCTION estado_deposito (@prod_codigo char(8), @depo_codigo char(2)) RETURNS NVARCHAR(200)
BEGIN
...
CREATE FUNCTION stock_fecha (@prod_codigo char(8), @fecha smalldatetime) RETURNS int
...
caso que sea necesario. Se sabe que debería existir un único gerente general (es decir,
debería ser el único empleado sin jefe). Si detecta que hay más de un empleado sin jefe
...
AS BEGIN
	...
	IF (@cant_emps_sin_jefe > 1)
		BEGIN
			/* selecciono el gerente tomando el primer elemento de la tabla */
			SELECT TOP 1 @jefe_codigo = empl_codigo
				FROM @emps_sin_jefe

			/* actualizo todos los empleados que no tengan jefe poninedole como jefe
			el gerente que yo elegi en el paso anterior */
			UPDATE Empleado
...
CREATE FUNCTION ver_gerente () RETURNS INT
BEGIN
	DECLARE @emp int
...
IF OBJECT_ID('compuesto_en_factura','P') IS NOT NULL
...
CREATE FUNCTION empleados_menores_a_cargo (@Jefe numeric(6)) returns int
AS BEGIN
	
	DECLARE @cant int
	DECLARE @jefe_nacimiento smalldatetime
	
	SELECT @jefe_nacimiento = empl_nacimiento 
		FROM Empleado 
		WHERE empl_codigo = @Jefe

	--sumamos los empleados a cargo del man con sus empleados a cargo
	SELECT @cant = isnull(sum(dbo.empleados_menores_a_cargo(empl_codigo)+1), 0) 
		FROM Empleado 
		WHERE empl_jefe = @Jefe AND empl_nacimiento > @jefe_nacimiento
...
CREATE FUNCTION empleados_a_cargo (@Jefe numeric(6)) returns int
AS BEGIN

	DECLARE @cant int

	--sumamos los empleados a cargo del man con sus empleados a cargo
	SELECT @cant = isnull(sum(dbo.empleados_a_cargo(empl_codigo)+1),0) 
		FROM Empleado 
...
CREATE FUNCTION jefe_mayor (@empleado numeric(6)) returns numeric(6)
AS BEGIN
